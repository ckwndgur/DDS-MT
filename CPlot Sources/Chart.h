...
public:
	...
	CString	GetDataTitle( int dataID ) { int i = GetIndex(dataID); if( i < 0 ) return (CString)""; else return (CString)(m_DataTitle[i]); }
...
public:
	...
	void	AddLineStylePopup( CMenu *addMenu, LPCSTR titleString, int dataID, int startVal );
