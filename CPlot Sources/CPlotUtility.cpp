...
CString	GetFontDescription( LOGFONT *font )
{
	...
	if( font == NULL ) return (CString)"";

	string.Format((CString)"%d point %s", font->lfHeight, font->lfFaceName );
	return string;
}
...
void	AddFontSizePopup( CMenu *addMenu, LPCSTR titleString, long compare, int startVal )
{
	CMenu			subMenu;
	int				i;
	CString			string;
	...
	for( i = 0; i < nFontSizes; i++ )
	{
		...
		string.Format((CString)"%d", fontSizes[i] );
...
void	AddColorTableMenu( CMenu *menu, UINT startVal )
{
	...
	while( curPos < length )
	{
		...
		newPos = string.Find((wchar_t)"\n", curPos );
		if( newPos < 0 ) newPos = length;

		// Extract the sub string
		if( newPos == curPos )
		{
			menu->AppendMenu( MF_SEPARATOR, 0, (CString)"0" );
		}
		else
		{
