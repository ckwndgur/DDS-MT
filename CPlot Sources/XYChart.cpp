...
int		CXYChart::ShowPopupMenu( CMenu *passedMenu, CWnd *window, UINT nFlags, CPoint point )
{
	...
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMajorHorizGrids, (CString)"Major Horizontal Grids" );
	flag = GetPopupCheckedFlag(this->m_UseMajorVerticalGrids);
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMajorVertGrids, (CString)"Major Vertical Grids" );
	flag = GetPopupCheckedFlag(this->m_UseMinorHorizontalGrids);
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMinorHorizGrids, (CString)"Minor Horizontal Grids" );
	flag = GetPopupCheckedFlag(this->m_UseMinorVerticalGrids);
	gridSubMenu.AppendMenu( MF_STRING | flag, kMenuMinorVertGrids, (CString)"Minor Vertical Grids" );
	menuPtr->AppendMenu( MF_POPUP | MF_STRING, (UINT) gridSubMenu.GetSafeHmenu(), (CString)"Grids" );
	}

	// Separator
	menuPtr->AppendMenu( MF_SEPARATOR, 0, (CString)"0" );

	// Check for a title, and put in title size if needed
	if( this->m_Title != "" )
	{
		AddFontSizePopup( menuPtr, "Title size", this->m_TitleFont.lfHeight, kMenuTitleFont );
	...
	// Settings...
	menuPtr->AppendMenu( MF_STRING, kMenuPlotSettings, (CString)"Settings..." );
...
void	CXYChart::AddDataPopupMenus( int whichDataSet, CMenu *menu, int startVal )
{
	...
	AddChartTypePopup( &dataMenu, "Chart type", this->GetChartType( dataID ), startVal+kMenuChartType );

	AddLineSizePopup( &dataMenu, "Line size", dataID, startVal + kMenuLineSize );

	AddLineStylePopup( &dataMenu, "Line style", dataID, startVal + kMenuLineStyle );

	dataMenu.AppendMenu( MF_SEPARATOR, 0, (CString)"0" );

	AddMarkerPopup( &dataMenu, "Marker", dataID, startVal + kMenuMarker );

	string = GetDataTitle( dataID );

	if( string == "" )
	{
		string.Format((CString)"Data set %d of %d", whichDataSet + 1, GetDataSetCount() );
	}
	else
	{
	...
	// Add it to main menu
	dataMenu.AppendMenu( MF_SEPARATOR, 0, (CString)"0" );
	dataMenu.AppendMenu( MF_STRING, startVal+kMenuDataRange-1, (CString)"Settings..." );

	menu->AppendMenu( MF_POPUP, (UINT) dataMenu.GetSafeHmenu(), string );
}

// Add a popup menu for the different chart types
void	CXYChart::AddChartTypePopup( CMenu *addMenu, LPCSTR titleString, long compare, int startVal )
{
	UINT			flag;

	flag = GetPopupCheckedFlag( compare == kXYChartPlain );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYChartPlain, (CString)"XY Scatter" );
	flag = GetPopupCheckedFlag( compare == kXYChartConnect );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYChartConnect, (CString)"XY Line" );
	flag = GetPopupCheckedFlag( compare == kXYChartHistogram );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYChartHistogram, (CString)"Histogram" );
}

// Add a popup menu for the different marker types and frequencies
void	CXYChart::AddMarkerPopup( CMenu *addMenu, LPCSTR titleString, int dataID, int startVal )
{
	CMenu			subMenu;
	...
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerNone, (CString)"No marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerCircle );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerCircle, (CString)"Circle marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerSquare );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerSquare, (CString)"Square marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerTriangle );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerTriangle, (CString)"Triangle marker" );
	flag = GetPopupCheckedFlag( markerShape == kXYMarkerX );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kXYMarkerX, (CString)"X marker" );

	addMenu->AppendMenu( MF_SEPARATOR, 0, (CString)"0" );

	flag = GetPopupCheckedFlag( GetMarkerFillState( dataID ) );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kMenuMarkerFill, (CString)"Filled marker" );
	...
	for( i = 0; i < nMarkerSizes; i++ )
	{
		...
		string.Format((CString)"%d", i+1 );
		subMenu.AppendMenu( MF_STRING | flag, startVal+i+kMenuMarkerSize, string );
	}
	addMenu->AppendMenu( MF_POPUP, (UINT) subMenu.GetSafeHmenu(), (CString)"Marker size" );

	// Marker frequency
	freq = abs(GetMarkerFrequency( dataID ));
	if( freq == 1 ) freq = 10;
	flag = GetPopupCheckedFlag( GetMarkerFrequency( dataID ) == 1 );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kMenuMarkerFrequency, (CString)"Mark every point" );
	flag = GetPopupCheckedFlag( (GetMarkerFrequency( dataID )) > 1 );
	string.Format((CString)"Mark every %d points", freq );
	...
	string.Format((CString)"Mark %d points", freq );
	addMenu->AppendMenu( MF_STRING | flag, startVal+kMenuMarkerFrequency+2, string );
}

// Add a popup for the line size of a data set
void	CXYChart::AddLineSizePopup( CMenu *addMenu, LPCSTR titleString, int dataID, int startVal )
{
	CMenu		subMenu;
	int			i;
	UINT		flag;
	...
	addMenu->AppendMenu( MF_POPUP, (UINT) subMenu.GetSafeHmenu(), (CString)titleString );
}

// Add a popup for the line size of a data set
void	CXYChart::AddLineStylePopup( CMenu *addMenu, LPCSTR titleString, int dataID, int startVal )
{
	CMenu		subMenu;
	UINT		flag;
	CString		string;
	...
	subMenu.AppendMenu( MF_STRING | flag, startVal, (CString)"Solid ____________" );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 1 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+1, (CString)"Dashed _ _ _ _ _ _" );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 2 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+2, (CString)"Dotted . . . . . ." );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 3 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+3, (CString)"Dash-Dot _ . _ . _" );
	flag = GetPopupCheckedFlag( GetDataLineStyle( dataID ) == 4 );
	subMenu.AppendMenu( MF_STRING | flag, startVal+4, (CString)"Dash-Dot-Dot _ . ." );
