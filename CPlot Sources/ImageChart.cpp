...
CRect	CImageChart::AdjustPlotSizeForWindowSize( CDC *pDC, CRect destRect, CRect plotRect )
{
	...
	if( m_PlotToWindowSize == TRUE )
	{
		if( m_MaintainAspectRatio == TRUE )
		{
			...
			ratio = fabs( minSize.cx ) / fabs( minSize.cy );
...
int		CImageChart::ShowPopupMenu( CMenu *passedMenu, CWnd *window, UINT nFlags, CPoint point )
{
	...
	menuPtr->AppendMenu( MF_STRING | flag, kMenuScaleToWindow, (CString)"Scale to window" );
	flag = GetPopupCheckedFlag(this->m_MaintainAspectRatio);
	menuPtr->AppendMenu( MF_STRING | flag, kMenuMaintainAspectRatio, (CString)"Maintain aspect ratio" );
...

	{
		...
		menuPtr->AppendMenu( MF_POPUP | MF_STRING, (UINT) ctSubMenu.Detach(), (CString)"Color Table" );
	}
