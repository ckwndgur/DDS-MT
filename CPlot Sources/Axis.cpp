...
void	CAxis::SetDefaults( void )
{
	...
	font1.CreateFont(24, 0, 0, 0, FW_BOLD, FALSE, FALSE, 0,
		...
		DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN, (CString)"Arial");
...
void	CAxis::OnDraw( CDC *pDC, CRect clientRect, CRect chartRect, CRect updateRect )
{
	...
	{ // Title
		long			height;
		CFont			font, *pOldFont, *pOldFont2=  nullptr;
...
void	CAxis::AddAxisPopupMenus( CMenu *menu, int startVal )
{
	...
	axisMenu.AppendMenu( MF_STRING | flag, startVal, (CString)"Auto range" );

	// Font sizes
	AddFontSizePopup( &axisMenu, "Title size", this->m_TitleFont.lfHeight, startVal+kMenuAxisTitleSize );
	AddFontSizePopup( &axisMenu, "Label size", this->m_LabelFont.lfHeight, startVal+kMenuAxisLabelSize );

	axisMenu.AppendMenu( MF_SEPARATOR, 0, (CString)"0" );

	// Number of major and minor ticks
	AddNumberOfTicksPopup( &axisMenu, "Major ticks", m_TickCount+1, startVal + kMenuAxisMajorTickCount );
	AddNumberOfTicksPopup( &axisMenu, "Minor ticks", m_MinorTickCount, startVal + kMenuAxisMinorTickCount );
	...
	axisMenu.AppendMenu( MF_STRING, startVal+kMenuAxisRange-1, (CString)"Settings..." );
...
void	CAxis::AddNumberOfTicksPopup( CMenu *addMenu, LPCSTR titleString, long compare, int startVal )
{
	...
	subMenu.AppendMenu( MF_STRING | flag, startVal, (CString)"Auto" );

	for( i = 0; i < nTickSizes; i++ )
	{
		flag = GetPopupCheckedFlag( compare == tickSizes[i] );
		string.Format((CString)"%d", tickSizes[i] );
