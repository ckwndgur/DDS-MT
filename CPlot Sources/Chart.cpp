								...
								int *nTicks, int *nMinor, double margin, double wasted )
{
	...
	{
		interval = pow( 10, rexp );
		minorTick = 9;
	}
	else if( mantissa >= cutPoint2 )
	{
		interval = 0.5 * pow( 10, rexp );
		minorTick = 4;
	}
	else
	{
		interval = pow( 10, rexp-1 );
		minorTick = 9;
	}
...
BOOL	CChart::WriteTextFile( CString pathName )
{
	...
	for( whichDataSet = 0; whichDataSet < GetDataSetCount(); whichDataSet++ )
	{
		...
		else
		{
			...
			string.Format((CString)"Cannot save files of %d dimensions.", dataSet->GetNDimensions() );
